...
(function(){
  ...
  const CACHE_VERSION = '27';  // ★ Task 26: unblock ALL fixes — collectCourses scope, Meggy AI, worker timeout, split CSS  // ★ Task 23: 4-issue fix — (1) decode %20 in course tiles via defensive decodeURIComponent in gdiAddCourseFromDrive/doAddCourseFromDrive/renderCourseCard (so legacy localStorage entries with URL-encoded names like "TJ%20SP%20-%20Black%20Edition" display decoded); (2) M13 Continue Card moved from afterbegin to beforeend on .gdi-wrap so it appears BELOW the folder list (was above breadcrumb, covering it); (3) added @media(max-width:480px) for .gdi-course tiles in gdi-study.js CSS — stats wrap 2x2, buttons shrink, padding reduced, so tiles don't deform on Android; (4) renderResumos in gdi-meggy.js now async + reads from BOTH localStorage AND Drive (.meggy.ai/resumos/) via GDIStorage.listMaterials, merges with dedup, lazy-loads Drive content on click — resumos generated by battalion on other devices now visible. Bumped 16→18 (jumped past 17 since Task 22 didn't actually bump)
