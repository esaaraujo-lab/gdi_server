...
if(!UI.disable_player){
  if(player_config.player==="plyr"){
    ...
    r=`<video id="player" playsinline controls autoplay muted data-poster="${n}" style="width:100%;height:100%;"><source src="${a}" type="${s?"application/x-mpegURL":"video/mp4"}">${o.map(h=>`<track kind="subtitles" src="${escHtml(h.url)}" label="${escHtml(h.label)}" default>`).join("")}</video>`;
    p="https://cdn.jsdelivr.net/npm/shaka-player@4.11.2/dist/shaka-player.compiled.js";
  }
}
...
function initPlayerEvents(){
  if(player_config.player==="plyr"){
    ...
    // Shaka Player init with keyboard shortcuts (space/f/m/arrows)
    ...
    try{
      ...
      if(shaka.Player.isBrowserSupported()){
        ...
        m.load(a).then(()=>{v.play().catch(()=>{});setTimeout(()=>{v.muted=false;},2000);}).catch(e=>{console.warn('[Shaka] load fail',e);});
        ...
        v.addEventListener("keydown",function(ev){
          ...
          else if(ev.key==="f")document.fullscreenElement?document.exitFullscreen():v.requestFullscreen();
      ...
      }else{console.warn('[Shaka] browser not supported');}
    }catch(e){console.warn('[Shaka] init fail',e);}
  }
}
