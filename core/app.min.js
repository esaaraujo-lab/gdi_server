...
// PATCH D: cache LRU com TTL 5min (300000ms) + cap 50 entradas; não invalida em page:change
const _listCache=new Map();const LIST_TTL=5*60*1000;const LIST_MAX=50;
function _listCacheGet(k){const e=_listCache.get(k);if(!e)return null;if(Date.now()-e.at>LIST_TTL){_listCache.delete(k);return null;}_listCache.delete(k);_listCache.set(k,e);return e.files;}
function _listCacheSet(k,f){if(_listCache.size>=LIST_MAX)_listCache.delete(_listCache.keys().next().value);_listCache.set(k,{at:Date.now(),files:f.slice()});}
async function gdiListAllFiles(path,pw,onPage){
  try{
    const key=path+'|'+(pw||'');
    const cached=_listCacheGet(key);
    if(cached){
      if(onPage)try{onPage(cached.slice())}catch(_){}
      return cached.slice();
    }
    ...
    for(let guard=0;guard<50;guard++){
      ...
      if(!files)break;
      for(let _i=0;_i<files.length;_i+=1000){
        const _chunk=files.slice(_i,_i+1000);
        for(let _j=0;_j<_chunk.length;_j++) out.push(_chunk[_j]);
      }
      if(onPage)try{onPage(out)}catch(_){}
      if(!page.nextPageToken)break;
      token=page.nextPageToken;idx++;
    }
    _listCacheSet(key,out);
    return out;
...
<footer class="gdi-footer"${UI.hide_footer?' style="display:none;"':""}>
  ...
  <span>Feito com &#10084; pela Meggy &middot; v${window.APP_VERSION||'1.0.0'} &middot; ${window.BUILD_DATE||''}</span>
</footer>`;$("body").html(i)}
...
function render(i){Bus.reset();if(i.indexOf("?")>=0)i=i.substr(0,i.indexOf("?"));title(i),nav(i);const e=/\/\d+:$/g;const done=()=>{try{Bus.emit('page:change')}catch(_){}};window.scroll_status={event_bound:!1,loading_lock:!1};if(i.includes("/fallback")){const t=getQueryVariable("a"),n=decodeURIComponent(getQueryVariable("id")||"");if(t){fallback(n,!0).then(done).catch(done);return}list(null,n,!0);done();return}if(window.MODEL&&window.MODEL.is_search_page){render_search_result_list();done();return}if(i==="/" ){list("/0:/");done();return}if(i.match(e)||i.slice(-1)=="/"){list(i);done();return}file(i).then(done).catch(done)}
...
// PATCH B: initFolderFilter delegado em document (bind único) + debounce 200ms
let _folderFilterDelegated=!1;
function initFolderFilter(){
  if(_folderFilterDelegated)return;
  _folderFilterDelegated=!0;
  let _t=null;
  document.addEventListener('input',function(e){
    if(!e.target||e.target.id!=='folder-filter')return;
    const q=e.target.value.toLowerCase();
    clearTimeout(_t);
    _t=setTimeout(function(){
      const rows=document.querySelectorAll('#list .gdi-row');
      rows.forEach(function(r){
        const n=(r.dataset.name||r.textContent||'').toLowerCase();
...
// PATCH C: guard evita acúmulo de listeners; reset só em page:change
let _sortState={col:null,dir:1};
let _sortBound=!1;
function initColumnSort(){
  if(_sortBound)return;
  _sortBound=!0;
  const i=document.querySelectorAll("#list-header .gdi-sort-header");
  i.forEach(e=>{e.addEventListener("click",function(){const t=this.dataset.sort;_sortState.col===t?_sortState.dir*=-1:(_sortState.col=t,_sortState.dir=1),i.forEach(n=>n.classList.remove("asc","desc")),this.classList.add(_sortState.dir===1?"asc":"desc"),sessionStorage.setItem("gdi-sort",JSON.stringify(_sortState)),sortFileList(t,_sortState.dir)})});
  try{const e=JSON.parse(sessionStorage.getItem("gdi-sort"));e&&e.col&&(_sortState=e)}catch{}
}
Bus.onGlobal('page:change',()=>{_sortBound=!1;});
...
// PATCH A + H + P.1: append sem re-parse + contadores no loop + paginação 50 itens por pasta
const LIST_PAGE_SIZE=50;
function _gdiLoadMoreHTML(remaining){return `<div id="gdi-load-more" style="text-align:center;padding:14px;"><button class="gdi-btn gdi-btn-ghost" type="button" id="gdi-load-more-btn" style="width:100%;">Carregar mais (${remaining} restantes)</button></div>`;}
function appendUnifiedRows(mode,items,basePath){
  ...
  // PATCH H: contadores no próprio loop (sem jQuery .find depois)
  let _totalBytes=0,_totalCount=0,_fileCount=0;
  // P.1: buffer de HTML por linha para paginação
  const _allRowHtml=[];
  for(const it of items){
    ...
    if(isFolder){
      ...
      _allRowHtml.push(`<a${attrs} class="gdi-row countitems" data-name="${escHtml(it.name.toLowerCase())}" data-date="${it.modifiedTime||""}">
  ...
  <span class="gdi-row-acts"></span>
    ...
    const bytes=Number(it.size)||0;_totalBytes+=bytes;_totalCount++;_fileCount++;it.size=formatFileSize(it.size);
    ...
    _allRowHtml.push(`<div class="gdi-row countitems size_items" data-name="${escHtml(it.name.toLowerCase())}" data-bytes="${bytes}" data-date="${it.modifiedTime||""}">
  ...
  // PATCH A: append sem re-parse + P.1: paginação 50 por pasta
  const isFirstPage=t.data("curPageIndex")=="0";
  if(isFirstPage){
    // nova pasta: reseta buffer e visível
    t.data("_allRowHtml",_allRowHtml);
    t.data("_visibleCount",0);
    t[0].innerHTML='';
  }else{
    // página adicional (scroll): anexa ao buffer existente
    const prev=t.data("_allRowHtml")||[];
    t.data("_allRowHtml",prev.concat(_allRowHtml));
  }
  const buf=t.data("_allRowHtml")||[];
  let visible=t.data("_visibleCount")||0;
  // renderiza próxima página (até LIST_PAGE_SIZE) se houver buffer
  if(visible<buf.length){
    const next=Math.min(buf.length,visible+LIST_PAGE_SIZE);
    t[0].insertAdjacentHTML('beforeend',buf.slice(visible,next).join(''));
    t.data("_visibleCount",next);
    visible=next;
  }
  // botão "Carregar mais" (P.1)
  let moreBtn=document.getElementById('gdi-load-more');
  if(visible<buf.length){
    if(!moreBtn){
      const listEl=t[0];
      const d=document.createElement('div');
      d.innerHTML=_gdiLoadMoreHTML(buf.length-visible);
      const el=d.firstElementChild;
      el.querySelector('#gdi-load-more-btn').addEventListener('click',function(){
        const buf2=t.data("_allRowHtml")||[];
        let vis2=t.data("_visibleCount")||0;
        if(vis2>=buf2.length)return;
        const nxt=Math.min(buf2.length,vis2+LIST_PAGE_SIZE);
        t[0].insertAdjacentHTML('beforeend',buf2.slice(vis2,nxt).join(''));
        t.data("_visibleCount",nxt);
        if(nxt>=buf2.length){
          const b=document.getElementById('gdi-load-more');
          if(b)b.remove();
        }else{
          const b=document.getElementById('gdi-load-more-btn');
          if(b)b.innerHTML=`Carregar mais (${buf2.length-nxt} restantes)`;
        }
      });
      listEl.parentNode.insertBefore(el,listEl.nextSibling);
    }else{
      const btn=moreBtn.querySelector('#gdi-load-more-btn');
      if(btn)btn.innerHTML=`Carregar mais (${buf.length-visible} restantes)`;
    }
  }else if(moreBtn){
    moreBtn.remove();
  }

  // PATCH B: initFolderFilter agora é delegado (bind único) — não re-bindar por página
  initColumnSort();
  initFolderFilter();

  if(isLastPage){
    // PATCH H: contadores vindos diretamente do loop (sem .find)
    const sizeStr=formatFileSize(_totalBytes)||"0 Bytes";
    const n=_totalCount,f=_fileCount;
    if(isSearch){
...
// v91: Shaka Player is the new DEFAULT (YouTube/Google Drive style skin).
// worker.js historically ships `player:'plyr'`; this client override forces
// Shaka unless the user has explicitly chosen another engine via the
// localStorage 'gdi-player-choice' key (set by external UI / settings panel).
// Other fields of window.player_config (version pins etc.) are preserved.
const UI=window.UI||{};
const _wpc=window.player_config||{};
let _forcedPlayer;
try{const _up=localStorage.getItem('gdi-player-choice');if(_up)_forcedPlayer=_up;}catch(_){}
const player_config=Object.assign({},_wpc);
player_config.player=_forcedPlayer||'shaka';
o=o||[];
...
if(!UI.disable_player){
  ...
  } else if(player_config.player==="shaka"){
    // v91: Shaka Player with YouTube/Google Drive style custom skin.
    // We render a wrapper (.shaka-video-container) + custom controls overlay
    // (.shaka-controls). The native `controls` attribute is intentionally
    // omitted so the two control bars don't compete.
    const _tracks=o.map(h=>`<track kind="subtitles" src="${escHtml(h.url)}" label="${escHtml(h.label)}" default>`).join("");
    r=`<div class="shaka-video-container" style="position:relative;background:#000;border-radius:12px;overflow:hidden;aspect-ratio:16/9;">
  <video id="player" playsinline autoplay muted data-poster="${n}" style="width:100%;height:100%;display:block;">
    <source src="${a}" type="${s?"application/x-mpegURL":"video/mp4"}">${_tracks}
  </video>
  <div class="shaka-controls">
    <div class="shaka-seek-bar"><div class="shaka-seek-bar-filled" style="width:0%;"></div></div>
    <div class="shaka-controls-row" style="display:flex;align-items:center;gap:12px;margin-top:8px;">
      <button type="button" class="shaka-play-button" title="Play/Pause" aria-label="Play/Pause">&#9654;</button>
      <button type="button" class="shaka-mute-button" title="Mute/Unmute" aria-label="Mute">&#128266;</button>
      <div class="shaka-volume-bar" title="Volume"><div class="shaka-volume-bar-filled" style="width:100%;"></div></div>
      <span class="shaka-time">0:00 / 0:00</span>
      <div style="flex:1;"></div>
      <button type="button" class="shaka-fullscreen-button" title="Fullscreen" aria-label="Fullscreen">&#9974;</button>
    </div>
  </div>
</div>`;
    p="https://cdnjs.cloudflare.com/ajax/libs/shaka-player/4.7.0/shaka-player.compiled.js";
  }
}
...
<div class="gdi-study" id="gdi-study">
 ...
 $("#content").html(f);
 // avisa módulos (gdi-ui.js etc.) que os slots do layout estão prontos p/ preencher
 try{Bus.emit('slots:ready');}catch(_){}

const LAST_KEY=()=>window.location.pathname+(window.location.search||'');
const NOTE_KEY=()=>{
  // ★ v1.0.84: distinguish search-result videos on the /fallback page.
  // Previously returned window.location.pathname ("/fallback") for ALL search-result videos,
  // so they shared watched/notes state — watching one marked ALL as watched.
  // Now we include the `id` query param to disambiguate.
  const p=window.location.pathname;
  if(p==='/fallback'||p.indexOf('/fallback')===0){
    let id='';
    try{id=getQueryVariable('id')||'';}catch(_){}
    return '/fallback::'+id;
  }
  return p;
};
...
// PATCH I: MutationObserver no lugar do polling de 40×250ms (one-shot, desconecta após 10s)
function gdiAnnounceVideoFromDOM(){
  const wrap=document.querySelector('.gdi-player-wrap');
  if(!wrap){
    // fallback: player nativo sem wrap — tenta direto
    const v0=document.querySelector('video');
    if(v0)gdiAnnounceMedia('video',v0);
    return;
  }
  ...
  const obs=new MutationObserver(function(){
    ...
    if(v2){gdiAnnounceMedia('video',v2);obs.disconnect();}
  });
...
// v91: YouTube/Google Drive style skin for Shaka Player (custom controls).
// Injected once per page; safe to call repeatedly.
function _gdiInjectShakaSkinCSS(){
  if(window.__gdiShakaSkinInjected)return;
  window.__gdiShakaSkinInjected=true;
  const css=`/* YouTube/Google Drive style Shaka Player skin (v91) */
.shaka-video-container{position:relative;background:#000;border-radius:12px;overflow:hidden;}
.shaka-video-container video{width:100%;height:100%;display:block;}
.shaka-controls{position:absolute;bottom:0;left:0;right:0;background:linear-gradient(transparent,rgba(0,0,0,0.7));padding:12px 16px;opacity:0;transition:opacity 0.3s;}
.shaka-video-container:hover .shaka-controls{opacity:1;}
.shaka-play-button{background:#ff0000;border:0;border-radius:50%;width:48px;height:48px;cursor:pointer;display:flex;align-items:center;justify-content:center;color:#fff;font-size:18px;line-height:1;padding:0;transition:background 0.2s,transform 0.1s;}
.shaka-play-button:hover{background:#cc0000;transform:scale(1.05);}
.shaka-seek-bar{height:4px;background:rgba(255,255,255,0.3);border-radius:2px;cursor:pointer;position:relative;overflow:hidden;}
.shaka-seek-bar:hover{height:6px;transition:height 0.15s;}
.shaka-seek-bar-filled{height:100%;background:#ff0000;border-radius:2px;width:0%;}
.shaka-time{color:#fff;font-size:12px;font-family:sans-serif;min-width:90px;text-align:center;}
.shaka-volume-bar{width:60px;height:4px;background:rgba(255,255,255,0.3);border-radius:2px;cursor:pointer;position:relative;overflow:hidden;}
.shaka-volume-bar-filled{height:100%;background:#fff;border-radius:2px;width:100%;}
.shaka-fullscreen-button,.shaka-mute-button{background:transparent;border:0;color:#fff;cursor:pointer;font-size:18px;padding:4px 8px;line-height:1;}
.shaka-fullscreen-button:hover,.shaka-mute-button:hover{color:#ff0000;}
.shaka-video-container video:focus{outline:none;}
`;
  const style=document.createElement('style');
  style.setAttribute('data-gdi-shaka-skin','v91');
...
function initPlayerEvents(){
  if(player_config.player==="plyr"){
    ...
    // v91: Shaka Player init + YouTube/Google Drive style custom skin wiring.
    // The custom controls overlay (.shaka-controls) is built in the HTML
    // render above. Here we wire it to the underlying <video> element AND
    // to the Shaka instance (when available). All controls work even if
    // Shaka fails to load — the native <video><source> fallback then plays.
    _gdiInjectShakaSkinCSS();
    const v=document.getElementById("player");
    const container=v?v.closest('.shaka-video-container'):null;
    function _fmtTime(t){if(!isFinite(t)||isNaN(t))return'0:00';const m=Math.floor(t/60),s=Math.floor(t%60);return m+':'+(s<10?'0':'')+s;}
    function _wireShakaControls(videoEl, wrapEl){
      if(!videoEl||!wrapEl||wrapEl.__gdiShakaWired)return;
      wrapEl.__gdiShakaWired=true;
      const playBtn=wrapEl.querySelector('.shaka-play-button');
      const muteBtn=wrapEl.querySelector('.shaka-mute-button');
      const fsBtn=wrapEl.querySelector('.shaka-fullscreen-button');
      const seekBar=wrapEl.querySelector('.shaka-seek-bar');
      const seekFilled=wrapEl.querySelector('.shaka-seek-bar-filled');
      const volBar=wrapEl.querySelector('.shaka-volume-bar');
      const volFilled=wrapEl.querySelector('.shaka-volume-bar-filled');
      const timeEl=wrapEl.querySelector('.shaka-time');
      if(playBtn)playBtn.addEventListener('click',()=>{videoEl.paused?videoEl.play().catch(()=>{}):videoEl.pause();});
      if(muteBtn)muteBtn.addEventListener('click',()=>{videoEl.muted=!videoEl.muted;muteBtn.innerHTML=videoEl.muted?'&#128263;':'&#128266;';});
      if(fsBtn)fsBtn.addEventListener('click',()=>{try{if(document.fullscreenElement){document.exitFullscreen();}else if(wrapEl.requestFullscreen){wrapEl.requestFullscreen();}else if(videoEl.requestFullscreen){videoEl.requestFullscreen();}}catch(_){}});
      if(seekBar)seekBar.addEventListener('click',ev=>{const r=seekBar.getBoundingClientRect();const p=Math.max(0,Math.min(1,(ev.clientX-r.left)/r.width));if(isFinite(videoEl.duration))videoEl.currentTime=p*videoEl.duration;});
      if(volBar)volBar.addEventListener('click',ev=>{const r=volBar.getBoundingClientRect();const p=Math.max(0,Math.min(1,(ev.clientX-r.left)/r.width));videoEl.volume=p;videoEl.muted=p===0;});
      videoEl.addEventListener('timeupdate',()=>{if(seekFilled&&isFinite(videoEl.duration)&&videoEl.duration>0)seekFilled.style.width=(videoEl.currentTime/videoEl.duration*100)+'%';if(timeEl)timeEl.textContent=_fmtTime(videoEl.currentTime)+' / '+_fmtTime(videoEl.duration);});
      videoEl.addEventListener('loadedmetadata',()=>{if(timeEl)timeEl.textContent=_fmtTime(videoEl.currentTime)+' / '+_fmtTime(videoEl.duration);});
      videoEl.addEventListener('volumechange',()=>{if(volFilled)volFilled.style.width=(videoEl.muted?0:videoEl.volume)*100+'%';if(muteBtn)muteBtn.innerHTML=videoEl.muted?'&#128263;':'&#128266;';});
      videoEl.addEventListener('play',()=>{if(playBtn)playBtn.innerHTML='&#10074;&#10074;';});
      videoEl.addEventListener('pause',()=>{if(playBtn)playBtn.innerHTML='&#9654;';});
      // initial state
      if(volFilled)volFilled.style.width=(videoEl.muted?0:(videoEl.volume||0))*100+'%';
      if(muteBtn)muteBtn.innerHTML=videoEl.muted?'&#128263;':'&#128266;';
      if(playBtn)playBtn.innerHTML='&#9654;';
    }
    if(v)try{v.tabIndex=0;}catch(_){}
    if(v)v.addEventListener("keydown",function(ev){
      if(ev.target&&ev.target.tagName==="INPUT")return;
      if(ev.key===" "){ev.preventDefault();v.paused?v.play().catch(()=>{}):v.pause();}
      else if(ev.key==="f"){try{document.fullscreenElement?document.exitFullscreen():(container||v).requestFullscreen();}catch(_){}}
      else if(ev.key==="m"){v.muted=!v.muted;}
      else if(ev.key==="ArrowRight")v.currentTime=v.currentTime+10;
      else if(ev.key==="ArrowLeft")v.currentTime=Math.max(0,v.currentTime-10);
      else if(ev.key==="ArrowUp")v.volume=Math.min(1,v.volume+.1);
      else if(ev.key==="ArrowDown")v.volume=Math.max(0,v.volume-.1);
    });
    try{
      shaka.polyfill.installAll();
      if(shaka.Player.isBrowserSupported()){
        const m=new shaka.Player(v);
        window._gdiPlayerInstance={
          _shaka:m,_el:v,
          muted:function(b){if(b===undefined)return v.muted;v.muted=b;},
          play:function(){return v.play();},
          pause:function(){v.pause();},
          setVolume:function(vol){v.volume=vol;},
          currentTime:function(t){if(t===undefined)return v.currentTime;v.currentTime=t;},
          duration:function(){return v.duration;}
        };
        if(container)_wireShakaControls(v,container);
        m.load(a).then(()=>{v.play().catch(()=>{});setTimeout(()=>{v.muted=false;},2000);}).catch(e=>{console.warn('[Shaka] load fail',e);if(container)_wireShakaControls(v,container);v.play().catch(()=>{});});
        v.addEventListener("ended",gdiQueueNext);
      }else{
        console.warn('[Shaka] browser not supported — using native <video>');
        if(container)_wireShakaControls(v,container);
        v.play().catch(()=>{});
        v.addEventListener("ended",gdiQueueNext);
      }
    }catch(e){
      console.warn('[Shaka] init fail',e);
      if(container)_wireShakaControls(v,container);
      v.play().catch(()=>{});
      v.addEventListener("ended",gdiQueueNext);
    }
...
if(p){
  if(window.Plyr||window.videojs||window.DPlayer||window.jwplayer||window.shaka){initPlayerEvents();gdiAnnounceVideoFromDOM();enableAutoplayWithUnmute();}
...
// P.2: playlist paginada (100 por página + "Carregar mais")
const PL_PAGE=100;
let _plVisible=0;
function _renderPlItem(m,v){
  ...
  return `<div class="gdi-playlist-item" data-idx="${v}" style="padding:8px 12px;margin:3px 0;border-radius:6px;cursor:pointer;display:flex;align-items:center;justify-content:space-between;font-size:13px;transition:background 0.2s;${y}">
...
function _plUpdateMoreBtn(listEl){
  let btn=document.getElementById('gdi-pl-more');
  if(_plVisible>=playlistVideos.length){
    if(btn)btn.remove();
    return;
  }
  if(!btn&&listEl){
    ...
    btn.innerHTML=`<button class="gdi-btn gdi-btn-ghost" type="button" id="gdi-pl-more-btn" style="width:100%;">Carregar mais (${playlistVideos.length-_plVisible} restantes)</button>`;
    listEl.parentNode.insertBefore(btn,listEl.nextSibling);
    btn.querySelector('#gdi-pl-more-btn').addEventListener('click',function(){
      const listEl2=document.getElementById("gdi-playlist-list");
      if(!listEl2)return;
      const next=Math.min(playlistVideos.length,_plVisible+PL_PAGE);
      listEl2.insertAdjacentHTML('beforeend',playlistVideos.slice(_plVisible,next).map((m,i)=>_renderPlItem(m,_plVisible+i)).join(''));
      _plVisible=next;
      _plUpdateMoreBtn(listEl2);
    });
  }else if(btn){
    const b=btn.querySelector('#gdi-pl-more-btn');
    if(b)b.innerHTML=`Carregar mais (${playlistVideos.length-_plVisible} restantes)`;
  }
}
function renderPlaylistUI(){
  ...
  // P.2: auto-expande a playlist se currentIndex estiver fora do intervalo visível
  if(currentIndex>=_plVisible){
    let target=Math.ceil((currentIndex+1)/PL_PAGE)*PL_PAGE;
    if(target<PL_PAGE)target=PL_PAGE;
    _plVisible=Math.min(playlistVideos.length,target);
  }
  // saneamento: se _plVisible stale (> que playlist) ou zero, reset
  if(_plVisible===0||_plVisible>playlistVideos.length){
    _plVisible=Math.min(playlistVideos.length,PL_PAGE);
  }
  if(listEl){
    listEl.innerHTML=playlistVideos.slice(0,_plVisible).map((m,i)=>_renderPlItem(m,i)).join('');
    _plUpdateMoreBtn(listEl);
  }
...
function switchVideo(h){
  ...
  else if(player_config.player==="shaka"&&window._gdiPlayerInstance&&window._gdiPlayerInstance._shaka){
    const shakaInst=window._gdiPlayerInstance._shaka;
    shakaInst.unload().then(()=>shakaInst.load(m.streamUrl)).then(()=>{
      if(window._gdiPlayerInstance._el) window._gdiPlayerInstance._el.play().catch(()=>{});
    }).catch(e=>console.warn('[Shaka] switchVideo load fail',e));
  }
...
// PATCH E: scanBatch paralelo (Promise.all, 6 concorrentes) + sem sleep 2s
async function loadCrossFolderPlaylist(onProgress){
  const INITIAL_ITEMS=60,BATCH_CONCURRENCY=6;
  ...
  async function scanBatch(count){
    const batch=subFolders.slice(cursor,cursor+count);
    cursor+=batch.length;
    const results=await Promise.all(batch.map(folder=>{
      const fpath=parentPath+encodeURIComponent(folder.name)+"/";
      const fpw=gdiGetPw(fpath);
      return gdiListAllFiles(fpath,fpw).then(files=>buildPlaylistFromFiles(files,fpath,folder.name));
    }));
    results.forEach(vids=>{if(vids&&vids.length)collected.push(...vids);});
    finalizeNames(collected);
    if(onProgress)try{onProgress(collected.slice(),cursor,subFolders.length)}catch(_){}
  }
  // primeira leva: 6 em paralelo até encher INITIAL_ITEMS
  while(cursor<subFolders.length&&collected.length<INITIAL_ITEMS){await scanBatch(BATCH_CONCURRENCY);}
  if(onProgress)try{onProgress(collected.slice(),cursor,subFolders.length)}catch(_){}
  // segunda leva: 6 em paralelo até o fim (sem sleep)
  while(cursor<subFolders.length){await scanBatch(BATCH_CONCURRENCY);}
  return dedupePlaylist(collected);
}

(async function buildPlaylist(){
try{
  // ★★★ GUARD CRÍTICO: não rodar buildPlaylist em páginas de pasta ★★★
  // Antes, o buildPlaylist rodava em TODAS as páginas. Em pastas com muitas
  // subpastas (ex: MATEMATICA com 16 cursos), ele escaneava TODAS as subpastas
  // via loadCrossFolderPlaylist, chamando renderPlaylistUI() a cada batch →
  // freeze permanente da thread principal.
  // Agora só roda em páginas de ARQUIVO (vídeo), onde a playlist é necessária.
  if(location.pathname.endsWith('/')){
    try{Bus.emit('playlist:ready', []);}catch(_){}
    return;
  }
  ...
  // PATCH G: emite evento p/ M20 substituir polling (listener em gdi-ui.js)
  try{Bus.emit('playlist:ready', playlistVideos);}catch(_){}
})();
}
...
const GDIUser=(()=>{
  ...
  // PATCH F: structuredClone ~10x mais rápido que JSON.parse(JSON.stringify)
  function mergeInto(local,remote){
    const m=(typeof structuredClone==='function')?structuredClone(local):clone(local);
    try{
  ...
  // PATCH F: debounce 1500→5000ms
  function schedule(){clearTimeout(saveTimer);saveTimer=setTimeout(_flush,5000);}
  // PATCH F: só re-fetch de /userstate a cada 5min (não a cada save)
  async function _flush(){
    if(!loaded||!state)return;
    if(Date.now()-(state._lastRemote||0)>5*60*1000){
      try{
        const r=await fetch('/userstate',{cache:'no-store'});
        if(r.ok){const remote=await r.json();if(remote&&typeof remote==='object')state=mergeInto(state,remote);state._lastRemote=Date.now();}
      }catch(_){}
    }
    try{
...
.gdi-player-wrap{border-radius:var(--ferreto-radius);overflow:hidden;background:#000;box-shadow:var(--ferreto-shadow);
  border:1px solid var(--ferreto-border);aspect-ratio:16/9;}
.gdi-player-wrap video{display:block;width:100%;height:100%;object-fit:contain;background:#000;}
...
@media(max-width:768px){
  ...
  .gdi-player-wrap{aspect-ratio:auto;max-height:50vh;}
  .gdi-player-wrap video{height:auto;max-height:50vh;}
...
`;document.head.appendChild(s);
console.log('[GDI BlackTie] layout injetado');
})();
