...
function render(i){Bus.reset();i.indexOf("?")>=0&&(i=i.substr(0,i.indexOf("?"))),title(i),nav(i);const e=/\/\d+:$/g;if(i.includes("/fallback")){window.scroll_status={event_bound:!1,loading_lock:!1};const t=getQueryVariable("a"),n=decodeURIComponent(getQueryVariable("id")||"");return t?fallback(n,!0):list(null,n,!0)}else window.MODEL?.is_search_page?(window.scroll_status={event_bound:!1,loading_lock:!1},render_search_result_list()):i==="/"?(window.scroll_status={event_bound:!1,loading_lock:!1},list("/0:/"),Bus.emit('page:change')):i.match(e)||i.slice(-1)=="/"?(window.scroll_status={event_bound:!1,loading_lock:!1},list(i)):file(i);Bus.emit('page:change')}
...
if(!UI.disable_player){
  if(player_config.player==="plyr"){
    ...
    // ★ v1.0.71: Shaka Player — suporte HLS/DASH com UI estilo YouTube
    r=`<video id="player" playsinline controls autoplay muted data-poster="${n}" style="width:100%;height:100%;">
      <source src="${a}" type="${s?"application/x-mpegURL":"video/mp4"}">
      ${o.map(h=>`<track kind="subtitles" src="${escHtml(h.url)}" label="${escHtml(h.label)}" default>`).join("")}
    </video>`;
    p="https://cdn.jsdelivr.net/npm/shaka-player@4.11.2/dist/shaka-player.compiled.js";
    g="https://cdn.jsdelivr.net/npm/shaka-player@4.11.2/dist/controls.css";
  }
}
...
function initPlayerEvents(){
  if(player_config.player==="plyr"){
    ...
    // ★ v1.0.71: Shaka Player init — UI estilo YouTube com suporte HLS/DASH
    const videoEl=document.getElementById("player");
    if(videoEl&&window.shaka){
      const player=new shaka.Player(videoEl);
      window._gdiPlayerInstance=videoEl; // usa o elemento video para compat
      player.load(a).then(()=>{
        videoEl.play().catch(()=>{});
        setTimeout(()=>{videoEl.muted=false;},2000);
      }).catch(e=>{console.warn('[Shaka] load error:',e);});
      videoEl.addEventListener("ended",gdiQueueNext);
      // Keyboard shortcuts (estilo YouTube)
      videoEl.tabIndex=0;
      videoEl.addEventListener("keydown",function(v){
        if(v.key===" "){v.preventDefault();videoEl.paused?videoEl.play():videoEl.pause();}
        else if(v.key==="f") document.fullscreenElement?document.exitFullscreen():videoEl.requestFullscreen();
        else if(v.key==="m") videoEl.muted=!videoEl.muted;
        else if(v.key==="ArrowRight") videoEl.currentTime+=10;
        else if(v.key==="ArrowLeft") videoEl.currentTime=Math.max(0,videoEl.currentTime-10);
        else if(v.key==="ArrowUp") videoEl.volume=Math.min(1,videoEl.volume+.1);
        else if(v.key==="ArrowDown") videoEl.volume=Math.max(0,videoEl.volume-.1);
      });
    }
  }
}
